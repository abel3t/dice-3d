...
(function () {
  ...
  const lLoc = [
    {
      lb_desc:
        'grrd’s Dice is a 3D Dice Roller app and a Yahtzee game for one to five players.',
    ...
    {
      lb_desc:
        'grrd’s Dice ist eine 3D Würfel-App und ein Yahtzee (Kniffel, Yatzy) Spiel für 1 bis 5 Spieler.',
    ...
    {
      lb_desc:
        'grrd’s Dice est une application 3D pour jouer aux dés et un jeu Yahtzee (Yatzee, Yatzy) pour 1 à 5 joueurs.',
    ...
    {
      lb_desc:
        "grrd's Dice is een 3D dobbelstenen app en een Yahtzee spel voor 1-5 spelers.",
    ...
    {
      lb_desc:
        "grrd's Dice è ina app 3D per far ir dats e dar Yahtzee per in enfin tschintg giugaders.",
  ...
  function shakeEventDidOccur() {
    ...
    for (i = 0; i < anzDices; i += 1) {
      if (!locked[i]) {
        targetRotationX[i] =
          (Math.round(
            ((targetRotationX[i] + 30 * Math.random() - 15) / Math.PI) * 2
          ) *
            Math.PI) /
          2;
        targetRotationY[i] =
          (Math.round(
            ((targetRotationY[i] + 30 * Math.random() - 15) / Math.PI) * 2
          ) *
            Math.PI) /
          2;
        targetRotationZ[i] =
          (Math.round(
            ((targetRotationZ[i] + 30 * Math.random() - 15) / Math.PI) * 2
          ) *
            Math.PI) /
          2;
      }
  ...
  function onDocumentMouseMove(event) {
    ...
    for (i = 0; i < anzDices; i += 1) {
      if (!locked[i]) {
        targetRotationX[i] =
          targetRotationXOnMouseDown[i] +
          (mouseX - mouseXOnMouseDown) * 0.08 * randX[i];
        targetRotationY[i] =
          targetRotationYOnMouseDown[i] +
          (mouseY - mouseYOnMouseDown) * 0.08 * randY[i];
        targetRotationZ[i] =
          targetRotationZOnMouseDown[i] +
          ((mouseX - mouseXOnMouseDown) * 0.08 * randZ[i] +
            (mouseY - mouseYOnMouseDown) * 0.08 * randZ[i]) /
            2;
      }
    }
  }
  ...
  function lockDice() {
    ...
    if (gWindowsHeight > gWindowsWidth) {
      ...
      if (
        mouseX > gWindowsHeight / -15 &&
        mouseX < gWindowsHeight / 15 &&
        mouseY > gWindowsHeight / -15 &&
        mouseY < gWindowsHeight / 15
      ) {
        locked[2] = !locked[2];
  ...
  function mouseOutMouseUp(event) {
    ...
    if (
      Math.abs(mouseX - mouseXOnMouseDown) < 5 &&
      Math.abs(mouseY - mouseYOnMouseDown) < 5 &&
      inYahtzee
    ) {
      inLock = true;
    ...
    for (i = 0; i < anzDices; i += 1) {
      if (!locked[i]) {
        targetRotationX[i] =
          (Math.round((targetRotationX[i] / Math.PI) * 2) * Math.PI) / 2;
        targetRotationY[i] =
          (Math.round((targetRotationY[i] / Math.PI) * 2) * Math.PI) / 2;
        targetRotationZ[i] =
          (Math.round((targetRotationZ[i] / Math.PI) * 2) * Math.PI) / 2;
      }
    }
    rolling = true;
  }
    ...
    if (event.touches.length === 1) {
      ...
      for (i = 0; i < anzDices; i += 1) {
        if (!locked[i]) {
          targetRotationX[i] =
            targetRotationXOnMouseDown[i] +
            (mouseX - mouseXOnMouseDown) * 0.06 * randX[i];
          targetRotationY[i] =
            targetRotationYOnMouseDown[i] +
            (mouseY - mouseYOnMouseDown) * 0.06 * randY[i];
          targetRotationZ[i] =
            targetRotationZOnMouseDown[i] +
            ((mouseX - mouseXOnMouseDown) * 0.06 * randZ[i] +
              (mouseY - mouseYOnMouseDown) * 0.02 * randZ[i]) /
              2;
        }
      }
    }
  ...
  function render() {
    ...
    for (i = 0; i < anzDices; i += 1) {
      ...
      curSpeed = Math.max(
        curSpeed,
        Math.abs(mesh[i].rotation.y - targetRotationX[i]) +
          Math.abs(mesh[i].rotation.x - targetRotationY[i]) +
          Math.abs(mesh[i].rotation.z - targetRotationZ[i])
      );
    }
      ...
      for (j = 0; j < 2; j += 1) {
        ...
        for (i = 0; i < anzDices; i += 1) {
          ...
          if ((rotX === 0 && rotY === 0) || (rotX === 2 && rotY === 2)) {
            ...
            diceVal[i] = 5;
          } else if ((rotX === 0 && rotY === 2) || (rotX === 2 && rotY === 0)) {
            diceVal[i] = 6;
          }
  ...
  function yahtzeeSetValue(e) {
    ...
    if (!empty && curPlayer === anzPlayer) {
      ...
      $helptit.innerHTML =
        lLoc[nLang].lb_player + ' ' + (i + 1) + ' ' + lLoc[nLang].lb_win;
      $help.innerHTML =
        lLoc[nLang].lb_with + ' ' + totalScore[i] + ' ' + lLoc[nLang].lb_pts;
  ...
  function setColor(e) {
    ...
    Array.from(document.getElementsByClassName('list-button-col')).forEach(
      function (rButton) {
  ...
  function fInit() {
    ...
    $('bt_dice').getElementsByTagName('span')[0].innerHTML = lLoc[
      nLang
    ].bt_dice.replace(/\s/g, '\u00a0');
    ...
    document
      ...
      .setAttribute('content', lLoc[nLang].lb_desc);
    ...
    Array.from(document.getElementsByClassName('todo')).forEach(function (
      rRow,
      index
    ) {
      j = index;
    ...
    Array.from(document.getElementsByClassName('list-button-50')).forEach(
      function (rButton) {
    ...
    Array.from(document.getElementsByClassName('list-button-20')).forEach(
      function (rButton) {
        rButton.addEventListener('click', function (e) {
          displayDice(e);
        });
    ...
    Array.from(document.getElementsByClassName('help')).forEach(function (
      rButton
    ) {
      rButton.addEventListener('click', function (e) {
